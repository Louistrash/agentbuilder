...
export function FeatureCard({
  ...
  return (
    <Card
      className={`p-4 sm:p-6 ${bgColor} rounded-xl border border-gray-800 hover:border-gray-700 transition-all duration-300 transform hover:-translate-y-1 hover:shadow-xl`}
      onMouseEnter={onMouseEnter}
      onMouseLeave={onMouseLeave}
    >
      <div 
        className={`${gradientClasses} rounded-lg h-32 sm:h-40 mb-4 sm:mb-6 flex items-center justify-center transform transition-all duration-500 ${
          isHovered ? 'scale-110 rotate-3' : ''
        }`}
      >
        <div className={`transform transition-all duration-300 ${
          isHovered ? 'scale-110 animate-bounce' : 'animate-pulse'
        }`}>
          {icon}
        </div>
      </div>
