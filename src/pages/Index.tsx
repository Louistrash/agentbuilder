...
const Index = () => {
  ...
  const fetchLogo = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('bot_settings').select('logo_url').single();
  ...
  return <div className="min-h-screen bg-luxury-50 flex flex-col">
      {/* Header */}
      <div className="bg-white/80 backdrop-blur-sm border-b border-luxury-100 p-4 sticky top-0 z-50">
        <div className="max-w-3xl mx-auto flex justify-between items-center px-[9px]">
          <div className="flex items-center gap-3">
            <img src={logoUrl || "/placeholder.svg"} alt="Archibot AI Logo" className="w-[70px] h-[70px] object-contain" />
          ...
          <div className="flex gap-2">
            {isAdmin && <Button variant="ghost" size="icon" onClick={() => navigate('/admin')} className="text-[#18344A]/80 hover:text-[#18344A]">
                <Settings className="h-4 w-4" />
              </Button>}
            <Button variant="ghost" size="icon" onClick={handleLogout} className="text-[#18344A]/80 hover:text-[#18344A]">
